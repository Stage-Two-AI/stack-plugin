...
/**
 * Kloont de template met alle tags in `doel`. Zonder token: een publieke repo over
 * https. `--filter=blob:none` haalt alleen de bestanden op die we echt bekijken; een
 * lokaal pad (tests) doet gewoon een volle kloon.
 */
...
export function branchHerbouwbaar(map, branch) {
  ...
  let commits;
  try {
    commits = git(map, "rev-list", `origin/${standaardBranch(map)}..origin/${branch}`).split("\n").filter(Boolean);
  } catch {
    return true;
  }
  return commits.every((sha) => git(map, "log", "-1", `--format=%(trailers:key=${TRAILER},valueonly)`, sha) !== "");
}
...
export function pnpmVoorKern() {
  return {
    lockfile(map) {
      try {
        sh("pnpm", ["install", "--lockfile-only", "--ignore-scripts"], { cwd: map, timeout: 180000 });
        return true;
      } catch (fout) {
...
/** Bewijst schrijfrecht zonder iets te schrijven. */
export function pushDroog(map, branch) {
  try {
    git(map, "push", "--dry-run", "-q", "origin", `HEAD:refs/heads/${branch}`);
    return true;
  } catch {
    return false;
  }
...
export function originUrl(map) {
  try {
    return git(map, "remote", "get-url", "origin");
  } catch {
    return null;
  }
}
 ...
 */
export function sluitOuderePRs({ repo, prefix, doelVersie, tekst }) {
  const gesloten = [];
  let open;
  try {
    open = openPRs(repo);
  ...
  for (const pr of open) {
    ...
    try {
      sh("gh", ["pr", "close", String(pr.number), "--repo", repo, "--delete-branch", "--comment", tekst]);
      gesloten.push(pr.headRefName);
    } catch {
...
export function templateMap(werkmap) {
  return join(werkmap, "template");
}
